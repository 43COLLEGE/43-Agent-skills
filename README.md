...
| 技能 | 一句话说明 | 配置门槛 |
|------|----------|----------|
| [feishu-assistant](./feishu-assistant/) | 用自然语言操作飞书：发消息、写文档、查日历、管知识库，不用再切来切去 | 需要飞书企业账号 |
| [social-media-scout](./social-media-scout/) | 抖音/小红书/B站等 10+ 平台的数据一句话查到，做竞品分析和选题研究不用手动翻 | 注册 API Key 即可 |
| [web-browser](./web-browser/) | 让 AI 操控真实 Chrome 浏览器——登录后台抓数据、搞定反爬平台、截图取证 | 需要 Node.js + Chrome |
| [media-transcriber](./media-transcriber/) | 丢个音视频文件进来，出逐字稿，能分清谁说了什么，开完会不用手动整理 | 安装 ffmpeg 即可 |
| [email-invoice-processor](./email-invoice-processor/) | 自动从邮箱里捞发票、识别金额税号、生成 Excel 汇总，报销季不用一封封翻 | 开启邮箱 IMAP 即可 |
| [chat-archiver](./chat-archiver/) | 和 AI 聊出了好东西？一句"入库"自动提炼精华、分类存进你的知识库 | 零配置可用 |
